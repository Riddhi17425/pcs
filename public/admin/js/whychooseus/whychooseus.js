$(document).ready(function () {
    ...
    var table = $('#whychooseus_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: window.APP_URLS.getWhychooseusdata,
        order: [[0, 'desc']],
        columns: [
            { data: 'id', name: 'id' },
    ...
    // delete milestone
    $(document).on('click', '.delete_whychooseus', function () {
        let id = $(this).data('id');

        var url = window.APP_URLS.deletewhychooseus.replace(':id' , id);
...
function validateAndPreviewImage() {
        const input = document.getElementById("whychooseus_image");
        const file = input.files[0];
        const errorSpan = document.getElementById("span_whychooseus_image");
        const previewImg = document.getElementById("preview_whychooseus_image");
        const bannerId = document.getElementById("span_whychooseus_image_id")?.value;
        ...
        if (!file) {
            errorSpan.innerText = "product image image is required.";
