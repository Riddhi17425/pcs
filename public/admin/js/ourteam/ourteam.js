$(document).ready(function () {
    ...
    var table = $('#ourteam_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: window.APP_URLS.getOurteamdata,
        order: [[0, 'desc']],
        columns: [
            { data: 'id', name: 'id' },
            { data: 'name', name: 'name' },
    ...
    // delete milestone
    $(document).on('click', '.delete_ourteam', function () {
        let id = $(this).data('id');

        var url = window.APP_URLS.deleteourteam.replace(':id' , id);
...
function validateAndPreviewImage() {
        const input = document.getElementById("ourteam_image");
        const file = input.files[0];
        const errorSpan = document.getElementById("span_ourteam_image");
        const previewImg = document.getElementById("preview_ourteam_image");
        const bannerId = document.getElementById("span_ourteam_image_id")?.value;
        ...
        if (!file) {
            errorSpan.innerText = "Team image is required.";
